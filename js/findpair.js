...
function Findpair(divid,output,config){
  ...
  fbox.newgame = function(){
    ...
    for(let i=0;i<rowarr.length;i++) {
      ...
      td1.style.backgroundImage = td2.style.backgroundImage = 'url("image/'+sarr[2]+'.png")';
    }
    ...
    for(let i=0;i<li.length;i++){
      li[i].addEventListener('click', function() {
        ...
        if(_this.success==total){
          ...
          ifbest += '。总共用了<span style="color:red">'+_this.step+'</span>步';
          // ifbest += '。<br>能达到的最少步数为<span style="color:red">'+_this.success+'</span>步。';
          xtip.win({
            type: 'alert',
            tip: '恭喜你游戏通关，获得神秘大礼包一份！预计这两天发放，请注意查收!'+ifbest,
            icon: 'l',
            times: 0,
            lock: true,
            shade: false,
